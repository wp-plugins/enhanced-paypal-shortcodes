...
Plugin URI: http://thewordpresswarrior.com/596/enhanced-paypal-shortcodes
...
define('TWPW_VERSION', '0.5');			// Current version of the Plugin
...
switch($atts['type']):
	case "paynow":	$code = '        <div style="';        if ($atts['textalign']) {              $code.='text-align: '.$atts['textalign'].';';        }
       ...
       $code.='</form>';	   	   $code.='</div>';break;
		...
		<input type="image" src="https://www.paypal.com/en_US/i/scr/pixel.gif" border="0" alt="" width="1" height="1" class="ppalholder">';
        if ($atts['imageurl']) {             $code.='<input type="image" src="'.$atts['imageurl'].'" border="0" name="submit" alt="'.ALT_ADD.'"';            if ($atts['imagewidth']){               $code.=' width="'.$atts['imagewidth'].'"';            }			$code.=' class="ppalbtn">';        } else {            $code.='<input type="image" src="https://www.paypal.com/en_AU/i/btn/btn_subscribeCC_LG.gif" border="0" name="submit" alt="PayPal - The safer, easier way to pay online." class="ppalbtn">';		}
